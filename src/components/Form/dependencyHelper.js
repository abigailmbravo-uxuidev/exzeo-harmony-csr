...
export default function dependencyHelper(question, data, values) {
  ...
  if (question.conditional.slider) {
    ...
    if (slider.minLocation) {
      updatedQuestion.leftLabel = toCurrency(
        Math.ceil(_.get(data, slider.minLocation))
      );
      updatedQuestion.min = Math.ceil(_.get(data, slider.minLocation));
    }
    if (slider.maxLocation) {
      updatedQuestion.rightLabel = toCurrency(
        Math.floor(_.get(data, slider.maxLocation))
      );
  ...
  if (question.conditional.value) {
    ...
    updatedQuestion.displayValue = toCurrency(
      value.type === 'percent'
        ? Math.ceil(calculatedValue / 100)
        : calculatedValue
    );
  }
  ...
  if (question.conditional.display && question.conditional.display.length > 0) {
    ...
    display.forEach(d => {
      const { type, trigger, parent, operator } = d;
